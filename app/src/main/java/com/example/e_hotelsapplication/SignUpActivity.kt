...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun signUp() {
        ...
        if (confirm.isEmpty()){
            binding.confirmpin.error = "Confirm password"
        }
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener { task->
                if (task.isSuccessful){
                    ...
                    startActivity(Intent(this@SignUpActivity, RestaurentListActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP))
                }else{
                    progressDialog.dismiss()
